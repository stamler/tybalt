...
exports.userRecordsMaster = [
  {
    ...
    email: "ttesterson@company.com",
    customClaims: { admin: true, standard: true },
  },
  {
    uid: "32517281",
    displayName: "Testacular Test",
    email: "ttest@company.com",
    customClaims: { standard: true },
  },
];
...
exports.makeFirestoreStub = (options = {}) => {
  ...
  if (certStrings) {
    getAzureDocSnapStub.withArgs("retrieved").returns({
      toDate: function () {
  ...
  docStub.returns({
    ...
    set: writeFail
      ? sinon.stub().throws(new Error("set to firestore failed"))
      : sinon.stub(),
    update: writeFail
      ? sinon.stub().throws(new Error("update to firestore failed"))
      : sinon.stub(),
  ...
  const commit = writeFail
    ? sinon.stub().throws(new Error("commit to firestore failed"))
    : sinon.stub();
...
exports.makeAuthStub = (options = {}) => {
  ...
  createCustomToken
    .withArgs(userRecords[0].uid)
    .returns(this.stubFirebaseToken);
  ...
  if (emailExists) {
    ...
    authStub = sinon.stub().returns({
      updateUser: uidExists
        ? sinon.stub().returns(userRecords[0])
        : sinon.stub().throws({ code: "auth/user-not-found" }),
      createUser: uidExists
        ? sinon.stub().throws({ code: "auth/uid-already-exists" })
        : sinon.stub().returns(userRecords[0]),
      createCustomToken,
      listUsers,
      getUser,
