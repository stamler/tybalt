...
interface DocIdObject {
  ...
  [propName: string]: any; // allow other properties
}
export function isDocIdObject(data: any): data is DocIdObject {  
  // if collectionName is present, it must be a string of length > 0
  if (data.collectionName !== undefined && (typeof data.collectionName !== "string" || data.collectionName.length < 1)) {
    return false;
  }
...
interface PurchaseOrderRequestObject {
  managerUid: string; // the uid of the manager
  managerName: string; // the displayName of the manager
  division: string;
  divisionName: string;
  paymentType: "Expense" | "OnAccount" | "CorporateCreditCard";
  total: number;
  vendorName: string;
  job?: string; // the job number
  jobDescription?: string;
  client?: string;
  jobCategory?: string;
  description: string;
  attachment?: string; // a string representing the download urls
  type: "normal" | "cumulative" | "recurring";
  endDate: number | Timestamp; // only present if type is recurring, value of Date.getTime()
}

export function isPurchaseOrderRequestObject(data: any): data is PurchaseOrderRequestObject {

  // it must contain a managerUid property which is a string longer than 16
  // characters
  if (typeof data.managerUid !== "string" || data.managerUid.length < 16) {
    functions.logger.error(`managerUid is not a string or is too short`);
    return false;
  }

  // it must contain a managerName property which is a string longer than 12
  // characters
  if (typeof data.managerName !== "string" || data.managerName.length < 5) {
    functions.logger.error(`managerName is not a string or is too short`);
    return false;
  }

  // it must contain a division property which is a string
  if (typeof data.division !== "string") {
    functions.logger.error(`division is not a string`);
    return false;
  }

  // it must contain a divisionName property which is a string longer than 3
  // characters
  if (typeof data.divisionName !== "string" || data.divisionName.length < 3) {
    functions.logger.error(`divisionName is not a string or is too short`);
    return false;
  }

  // it must contain a paymentType property which is one of the allowed values
  if (typeof data.paymentType !== "string" || 
    !["Expense", "OnAccount", "CorporateCreditCard"].includes(data.paymentType)) {
    functions.logger.error(`paymentType is not a string or is not one of the allowed values`);
    return false;
  }

  // it must contain a total property which is a positive number with no more
  // than 2 decimal places
  if (typeof data.total !== "number" ||
    data.total < 0 ||
    !(Math.floor(data.total) === data.total || // has decimal places
    data.total.toString().split(".")[1].length < 3 // more than 2 decimal places
    )) {
      functions.logger.error(`total is not a number or has more than 2 decimal places`);
      return false;
  }

  // it must contain a vendorName property which is a string longer than 3 chars
  if (typeof data.vendorName !== "string" || data.vendorName.length < 4) {
    functions.logger.error(`vendorName is not a string or is too short`);
    return false;
  }

  // if it has a job property, it must be a string longer than 5 chars
  if (data.job !== undefined && (typeof data.job !== "string" || data.job.length < 6)) {
    functions.logger.error(`job is not a string or is too short`);
    return false;
  }

  // if jobDescription is present, it must be a string longer than 3 chars and
  // job must be present
  if (data.jobDescription !== undefined && (typeof data.jobDescription !== "string" || data.jobDescription.length < 4 || data.job === undefined)) {
    functions.logger.error(`jobDescription is not a string or is too short`);
    return false;
  }

  // if client is present, it must be a string longer than 3 chars and job must
  // be present
  if (data.client !== undefined && (typeof data.client !== "string" || data.client.length < 3 || data.job === undefined)) {
    functions.logger.error(`client is not a string or is too short`);
    return false;
  }

  // if jobCategory is present, it must be a string longer than 3 chars and job
  // must be present
  if (data.jobCategory !== undefined && (typeof data.jobCategory !== "string" || data.jobCategory.length < 4 || data.job === undefined)) {
    functions.logger.error(`jobCategory is not a string or is too short`);
    return false;
  }

  // description must be a string longer than 8 chars
  if (typeof data.description !== "string" || data.description.length < 9) {
    functions.logger.error(`description is not a string or is too short`);
    return false;
  }

  // if attachment is present, it must be a string longer than 23 chars
  if (data.attachment !== undefined && (typeof data.attachment !== "string" || data.attachment.length < 24)) {
    functions.logger.error(`attachment is not a string`);
    return false;
  }

  // type must be one of the allowed values
  if (typeof data.type !== "string" || !["normal", "cumulative", "recurring"].includes(data.type)) {
    functions.logger.error(`type is not a string or is not one of the allowed values`);
    return false;
  }

  // if type is recurring, it must have an endDate property which is a number
  // that is the value of a Date.getTime() object
  if (data.type === "recurring" && (typeof data.endDate !== "number" || isNaN(data.endDate))) {
    functions.logger.error(`endDate is not a number`);
    return false;
  }
